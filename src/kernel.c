...
static void terminal_write(const char* text) {
    for (uint32_t i = 0; text[i] != '\0'; ++i) terminal_putchar(text[i]);
}

static void terminal_write_uint(uint32_t value) {
    char buffer[11]; uint32_t i = 0;
...
void kmain(void) {
    ...
    users_init();
    terminal_write("[users] Root account initialized (uid 0).\n");
    terminal_write("[sudo] Privilege subsystem initialized.\n");
    terminal_write("[sudo] Current user is authorized for administrative commands.\n\n");
    ...
    terminal_write("c-OS privilege model ready.\n");
